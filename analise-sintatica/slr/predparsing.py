...
class Grammar:
            ...
            for rhs in self.production_rules[s]:
                ...
                for i in range(1, len(rhs)):
                    ...
                    if "epsilon" in self.first_tab[y_ant]:
                        # Finally, we include FIRST(y_i) in FIRST(s).
    ...
    def first_log(self, symb, first, lhs, rhs):
        '''
        ...
        if not first in self.first_tab[symb]:
            self.first_trace.append("Added " + str(first) + " to FIRST(" + str(symb) + ") " + \
                                    "while processing rule " + str(lhs) + " -> " +  ''.join(rhs))
    ...
    def follow_log(self, symb, follow, lhs, rhs, n):
        '''
        ...
        if not follow.issubset(self.follow_tab[symb]):
            self.follow_trace.append("Added " + str(follow) + " to FOLLOW(" + str(symb) + ") " + \
                                     "while processing rule " + str(lhs) + " -> " +  ''.join(rhs) + ", caso: " + str(n))
    ...
    def compute_follow(self):
        ...
        while True:
            follow_size = self.follow_tab_size()
            for s in self.non_terminals:
                self.follow(s)
            new_follow_size = self.follow_tab_size()
            if new_follow_size == follow_size:
                break
            ...
            for i in range(len(rhs)):
                ...
                if B in self.non_terminals:
                    ...
                    if beta != ():
                        ...
                        first_beta_no_epsilon = first_beta - {"epsilon"}
                        self.follow_log(B, first_beta_no_epsilon, s, rhs, 1)                        
                        self.follow_tab[B] = \
                        self.follow_tab[B].union(first_beta_no_epsilon)
                        # If there is a production A -> alpha B beta
                        # where FIRST(beta) contains epsilon, then FOLLOW(A) \subset FOLLOW(B) 􏰛􏰂
                        if "epsilon" in first_beta:
                            print(f'B: {B}, follow: {self.follow_tab[s]}, rule: {s} -> {rhs}, case 2, first_beta: {first_beta}')
                            self.follow_log(B, self.follow_tab[s], s, rhs, 2)
                            self.follow_tab[B] = \
                            self.follow_tab[B].union(self.follow_tab[s])
                        else:
                            # If there is a production A -> alpha B
                            # then FOLLOW(A) \subset FOLLOW(B)
                            self.follow_log(B, self.follow_tab[s], s, rhs, 3)
                            self.follow_tab[B] = \
                            self.follow_tab[B].union(self.follow_tab[s])
            ...
            for alpha in self.production_rules[A]:
                ...
                if alpha == ("epsilon",) or \
                   (alpha != ("epsilon",) and ("epsilon" in self.firstW(alpha))):
                    ...
                    if "$" in self.follow_tab[A]:
                        self.pred_parsing_tab[A]["$"].append(str(A) + " -> " + ''.join(alpha))
                else:
...
#    ex.solve()
    ...
    # Production rules for Question 80 from Gradiance
    # S → aB
    # S → bA
    # S → ε
    # A → bAA
    ...
    ex = Exercise("Question 80 from Gradiance", g)
#    ex.solve()

    # Production rules for Question 81 from Gradiance    
    # S → ABCDE
    # A → a
    # B → b | ε
    # C → c | ε
    ...
    g = Grammar("S", p, ["S", "A", "B", "C", "D", "E"],
                ["a", "b", "c", "d", "e"])

    ex = Exercise("Question 81 from Gradiance", g)
    ex.solve()
